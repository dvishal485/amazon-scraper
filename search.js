        ...
        try {
            ...
            if (!link.includes('/gp/slredirect/')) { /* Not including sponsered products */
                result.push({
                    ...
                    current_price: parseFloat(all_product[i].split('<span class="a-price" data-a-size="l" data-a-color="price"><span class="a-offscreen">')[1].split('</span>')[0].replace(/,/g, '').replace('₹', '').trim()),
            ...
            try {
                ...
                if (!link.includes('/gp/slredirect/')) {
                    result.push({
                        ...
                        link,
                        current_price: parseFloat(all_product_container[i].split('<span class="a-price" data-a-size="l" data-a-color="price"><span class="a-offscreen">')[1].split('</span>')[0].replace(/,/g, '').replace('₹', '').trim()),
